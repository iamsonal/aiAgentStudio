...
- [ ] All catch blocks use `ActionOutcome.fromException(e)` — not `failure(ERR_CODE_UNEXPECTED_ERROR, ...)`
- [ ] Business-logic failures use explicit error codes (not `UNEXPECTED_ERROR`)
