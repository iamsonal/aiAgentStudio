...
<CardGrid>
  ...
  <Card title="Context Provider — Metadata" icon="seti:config">
    ...
    - [ ] `ProviderDefinition__c` points to the correct provider definition
    - [ ] `Scope__c` and `ApplicableObjects__c` are configured correctly
...
2. Verify `ProviderDefinition__c` points to an active `ContextProviderDefinition__mdt` record.
3. Confirm `Scope__c` and `ApplicableObjects__c` match your use case.
4. Check `Priority__c` and `OverlapMode__c` — earlier bindings may suppress later ones.
