...
<Aside type="note" title="Testing Convention">
In this repo, prefer the shared `TestFactory` builders for Apex tests instead of hand-rolling large data graphs. Focus tests on observable business outcomes, negative paths, and async behavior, not just line coverage.
</Aside>
...
|:------|:------|
| AI Agent Definition | (Your agent) |
| Provider Definition | `Account Context` |
| Scope | `Record` |
| Applicable Objects | `Account` |
| Priority | `10` |
| Failure Mode | `Warn` |
| Freshness Policy | `Session` |
| Overlap Mode | `Prefer Highest Priority` |
| Projection Mode | `Provider Default` |
| Is Active | `true` |
| Admin Label | `Account Context` |
| Provider Config Json | `{"caseLimit": 10, "activityDays": 30}` |

For user-centric providers:

| Field | Value |
|:------|:------|
| Provider Definition | `User Details Provider` |
| Scope | `User` |
| Priority | `20` |
| Failure Mode | `Warn` |
| Freshness Policy | `Current Only` |
| Overlap Mode | `Allow` |
| Projection Mode | `Provider Default` |
| Applicable Objects | (leave blank) |
...
Use `TestFactory` when your action depends on agent definitions, capabilities, executions, or richer data graphs. The lightweight examples below keep the setup minimal for clarity.
...
private class ActionCalculateAccountHealthTest {
    ...
    static void testSuccessfulExecution() {
        ...
        ActionContext ctx = new ActionContext(
            ...
            1,              // currentTurnCount
            'Conversational'  // execution strategy
        );
...
Use `Fail Fast` for agents where partial execution is worse than no execution (e.g., financial workflows, multi-step record mutations). Use `Autonomous` for iterative user-facing runtimes where the LLM recovering gracefully provides a better user experience.
...
<CardGrid>
  <Card title="Custom Action — Code" icon="seti:apex">
    ...
    - [ ] Tests assert business outcomes, negative paths, and async behavior
  </Card>
  ...
  <Card title="Context Provider — Code" icon="seti:db">
    ...
    - [ ] Tests cover empty anchors, permissions, and result shape
  </Card>
  <Card title="Context Provider — Metadata" icon="seti:config">
    - [ ] `AgentContextConfig__c` record created
    - [ ] `ContextDefinition__c` points to the correct context definition
    - [ ] The bound `ContextDefinition__mdt` record is active and uses the intended scope
    - [ ] `Priority__c` set for proper sequencing
    - [ ] `IsActive__c = true`
  </Card>
</CardGrid>
...
2. Verify `ContextDefinition__c` points to an active `ContextDefinition__mdt` record.
3. Confirm the definition's `DefaultScope__c` and mode match your use case.
4. Check `Priority__c` when multiple bindings exist for the same agent.
