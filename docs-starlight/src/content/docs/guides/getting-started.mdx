...
| Agent Type | `Iterative` |
| LLM Configuration | `OpenAI_GPT4o` |
| Is Active | ✓ |

<Aside type="note" title="Core Runtime Scope">
The runtime documented here uses execution strategies. Start with `Iterative` for multi-turn experiences, use `SingleShot` for one-turn automations, and `Pipeline` for sequential orchestration. Channel behavior such as email delivery is configured separately.
</Aside>
