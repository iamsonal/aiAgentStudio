...
The runtime documented here uses execution strategies. Start with `Conversational` for multi-turn experiences, use `Direct` for one-turn automations, and `Pipeline` for sequential orchestration. Channel behavior such as email delivery is configured separately.
</Aside>
