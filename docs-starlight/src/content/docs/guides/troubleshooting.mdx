...
<Steps>
1. Go to **Setup → External Credentials** and open `OpenAIEC` (or your provider-specific external credential).
2. Verify the principal is configured and the auth parameter, such as `OpenAIKey`, is populated.
3. Go to **Setup → Named Credentials** and confirm the `LLMConfiguration__c.NamedCredential__c` value points to an active named credential such as `OpenAI_API`.
</Steps>
...
<CardGrid>
  <Card title="Reduce History" icon="seti:clock">
    Lower `HistoryTurnLimit__c` (try 5–7). Switch to `Summary Buffer` memory strategy to compress older turns.
  </Card>
...
```text
Retrieve a contact by exact email address.
Use when the user already knows the email and needs the matching contact record.
Do NOT use for broad contact searches, account lookups, or contact creation.
```
...
**Other Fixes**
- **Lower Temperature** — reduce `DefaultTemperature__c` to 0.3–0.5 for more predictable tool selection
...
2. Verify `MemoryStrategy__c` is set (`Buffer Window` for precise context).
...
<CardGrid>
  <Card title="1. Use the Execution Storyboard" icon="seti:log">
    If your org includes the addon UI, open the **Execution Storyboard** for the affected `AgentExecution__c` record. Otherwise inspect `ExecutionStep__c` rows directly to see prompts, tool calls, and failures.
  </Card>
