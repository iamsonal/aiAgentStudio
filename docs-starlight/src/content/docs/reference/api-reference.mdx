...
If you do not want to memorize field names, think of the request in four parts:
...
2. Keep caller contracts stable and let the framework manage runtime state.
3. Preserve continuity explicitly when the caller cares about an ongoing conversation or thread.
4. When debugging, look at the execution, session, message history, and step trace together rather than in isolation.
</Steps>
