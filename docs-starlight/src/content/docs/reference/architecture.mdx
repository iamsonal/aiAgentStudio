...
AI Agent Studio is a governed AI runtime for Salesforce.
...
- an agent definition says what the agent is supposed to do
- an execution is one unit of runtime work
- a session is the longer-lived conversation or thread
- a message is what came in or went out through a channel
- an execution step is the detailed trail of what the runtime did
...
“ChatAgent,” “EmailAgent,” and “WhatsAppAgent.” Instead, runtime style and transport behavior stay
orthogonal.
...
For channel-based traffic, the framework first answers a handful of practical questions:
...
Once that context is clear, the main runtime starts. At that point the framework stops thinking
about "what came in" and starts thinking about "what kind of work should own this request."

That handoff is one of the most important design boundaries in the system.
...
Eventually the runtime reaches the model interaction stage.

That is where prompts, memory, context, tool definitions, masking, and safety controls come
together into one request.

When a result comes back, the framework decides whether the model:
...
If a tool needs to run, the framework hands off to the tool execution layer. A capability might map
to:
...
If the result should be visible to a user, the framework then applies delivery policy. Depending on
the channel and route, that may mean replying immediately, saving a draft, suppressing output, or
passing the result through a review flow.
...
Provider-backed channels such as WhatsApp and Slack are a good example of how the framework handles
transport-specific concerns without polluting the generic agent API.
...
The runtime also keeps track of what is still safe to do in the current transaction:
...
## The Most Important Runtime Responsibilities

If you ignore class names and focus on responsibilities, the framework is doing a small number of
important jobs:

- decide what kind of runtime should own the request
- keep continuity across sessions and messages
- build and send the model request
- run tools when the model asks for them
- record what happened in durable execution history
- decide how, when, or whether a response should be delivered

That is the shape of the system. The codebase has many classes, but they mostly exist to support
