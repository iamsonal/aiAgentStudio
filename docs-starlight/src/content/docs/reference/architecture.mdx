...
<CardGrid>
  <Card title="Orchestrators" icon="seti:settings">
    **Three agent types**: Conversational (multi-turn chat), Function (single-task with sync/async routing), Email (thread processing with auto-reply).

    All extend `BaseAgentOrchestrator` and implement `IAgentOrchestrator` interface.

    **Sequential Pipelines** are a separate standalone concept: `SequentialPipelineOrchestrator` chains multiple agents step-by-step via `PipelineDefinition__c` metadata — not an agent type, but a higher-level composition pattern.
  </Card>
...
2. **Orchestrator Routing**
   ...
   `AgentExecutionService` routes by agent type to the appropriate orchestrator (Conversational, Function, or Email). Sequential pipelines are started separately via `SequentialPipelineOrchestrator.startPipeline()`.
...
6. **Response Processing**
   ...
   - **Tool Calls**: `ToolCallResponseHandler` executes eligible tools (filtered by Tool Flow Graph if enabled), manages async dispatch
...
**Identity & Behavior**: `AgentType__c` (Conversational/Function/Email), `IdentityPrompt__c` and `InstructionsPrompt__c` (System prompt components), `EnableToolReasoning__c` (Require LLM to explain tool selection), `EnableParallelToolCalling__c` (Allow multiple tools per response).
...
**Performance**: `AsyncDispatchType__c` (High for Platform Events / Low for Queueable), `MaxProcessingCycles__c` (Max LLM turns per execution), `EnableToolFlowGraph__c` (Enforce Tool Flow Graph at runtime).

**Tool Flow**: `ToolFlowGraph__c` (JSON flow graph defining entry tools and typed transitions).
...
**Basic Config**: `CapabilityName__c` (Tool name exposed to LLM), `Description__c` (When and how to use this tool - critical for LLM guidance), `ImplementationType__c` (Standard/Apex/Flow/**Agent**), `StandardActionType__c` (Maps to StandardActionHandler__mdt), `ImplementationDetail__c` (Apex class name, Flow API name, or child agent's `DeveloperName__c` for Agent type).
...
**Turn Coordination**: `CurrentTurnIdentifier__c` (Optimistic concurrency token for the active turn; carried by async jobs to detect stale work), `AsyncToolTurnIdentifier__c` (Turn token for the in-flight async tool batch while awaiting completions), `OriginalTurnIdentifier__c` (Previous turn token when the current turn is rotated during resume/recovery).
...
**Step Metadata**: `StepType__c` (UserInput/AgentResponse/ToolCall/ToolResult/Error), `StepRole__c` (User/Assistant/Tool), `TurnIdentifier__c` (Groups steps by LLM turn; typically matches `AgentExecution__c.CurrentTurnIdentifier__c` while the turn is active).
...
<CardGrid>
  ...
  <Card title="Agent-as-Capability" icon="seti:git">
    Set `AgentCapability__c.ImplementationType__c = 'Agent'` and point `ImplementationDetail__c` at a child agent's developer name. The **LLM decides** when to invoke the sub-agent as a tool — the child runs in a separate transaction and its result flows back into the parent conversation.

    Always routes through `AsyncFunctionOrchestrator`. The child execution carries `TriggerSource__c = 'AgentCapability'` and a `ParentExecution__c` link.
  </Card>
  <Card title="Sequential Pipelines" icon="seti:flow">
    `SequentialPipelineOrchestrator` executes a fixed sequence of agents defined in `PipelineDefinition__c` / `PipelineStep__c` metadata. Each step spawns a child agent; completion advances to the next step. Progress tracked in `PipelineRunStep__c`.

    Use pipelines when the sequence is predetermined. Use Agent-as-Capability when the sub-agent invocation should be LLM-directed.
  </Card>
  <Card title="Tool Flow Graph" icon="seti:git">
    Declarative workflow control limits the LLM to only the tools eligible at the current step — preventing out-of-order calls and enabling deterministic branching.

    **Design-time**: Generate a flow graph (entry tools + typed transitions) via LLM analysis or manual editing in the visual editor.

    **Runtime**: `ToolFlowGraphService` computes the eligible tool set on every LLM call. Exclusive path locking ensures branch choices are permanent.
  </Card>
</CardGrid>
...
<CardGrid>
  <Card title="Storyboard UI" icon="seti:graph-line">
    `AgentStoryboardController` powers the LWC components (`agentStoryboard`, `agentExecutionView`, `workflowView`, `agentStoryboardStep`) with a visual timeline of the full execution, including pipeline and sub-agent hierarchies.
  </Card>
