...
**Response Modes**: Internally supports Block, Sanitize, Flag, LogOnly. Presets map to these modes for simplicity.

**Configuration**: Per-agent via `AIAgentDefinition__c` field `PromptSafetyPreset__c` (Off / Standard / Strict). Standard logs but does not block; Strict blocks on detection using all categories.
...
- [ ] Prompt safety mode set to Block or Sanitize (not LogOnly) for production agents
