...
function _formatJsonForDisplayInternal(data) {
    ...
    try {
        ...
        try {
            ...
            return String(data).substring(0, MAX_FALLBACK_LENGTH) + (String(data).length > MAX_FALLBACK_LENGTH ? '...[Unserializable Data]' : '[Unserializable Data]');
        } catch (strErr) {
...
export function formatDisplayMessages(messages, existingCount = 0) {
    ...
    return messages.map(msg => {
        ...
        const toolInfoAvailable = (role === 'assistant' && msg.toolCallsData) ||
